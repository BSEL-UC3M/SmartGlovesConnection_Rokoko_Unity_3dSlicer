...
class HAND_TRACKING(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.title = "HAND_TRACKING"  # TODO: make this more human readable by adding spaces
...
See more information in <a href="https://github.com/organization/projectname#HAND_TRACKING">module documentation</a>.
"""
...
def registerSampleData():
  ...
  SampleData.SampleDataLogic.registerCustomSampleDataSource(
    ...
    category='HAND_TRACKING',
    sampleName='HAND_TRACKING1',
    # Thumbnail should have size of approximately 260x280 pixels and stored in Resources/Icons folder.
    # It can be created by Screen Capture module, "Capture all views" option enabled, "Number of images" set to "Single".
    thumbnailFileName=os.path.join(iconsPath, 'HAND_TRACKING1.png'),
    # Download URL and target file name
    uris="https://github.com/Slicer/SlicerTestingData/releases/download/SHA256/998cb522173839c78657f4bc0ea907cea09fd04e44601f17c82ea27927937b95",
    fileNames='HAND_TRACKING1.nrrd',
    # Checksum to ensure file integrity. Can be computed by this command:
    #  import hashlib; print(hashlib.sha256(open(filename, "rb").read()).hexdigest())
    checksums = 'SHA256:998cb522173839c78657f4bc0ea907cea09fd04e44601f17c82ea27927937b95',
    # This node name will be used when the data set is loaded
    nodeNames='HAND_TRACKING1'
  )
  ...
  SampleData.SampleDataLogic.registerCustomSampleDataSource(
    # Category and sample name displayed in Sample Data module
    category='HAND_TRACKING',
    sampleName='HAND_TRACKING2',
    thumbnailFileName=os.path.join(iconsPath, 'HAND_TRACKING2.png'),
    # Download URL and target file name
    uris="https://github.com/Slicer/SlicerTestingData/releases/download/SHA256/1a64f3f422eb3d1c9b093d1a18da354b13bcf307907c66317e2463ee530b7a97",
    fileNames='HAND_TRACKING2.nrrd',
    checksums = 'SHA256:1a64f3f422eb3d1c9b093d1a18da354b13bcf307907c66317e2463ee530b7a97',
    # This node name will be used when the data set is loaded
    nodeNames='HAND_TRACKING2'
  )
...
class HAND_TRACKINGWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
  ...
  def setup(self):
    ...
    uiWidget = slicer.util.loadUI(self.resourcePath('UI/HAND_TRACKING.ui'))
    ...
    # self.DeliveryTrainingSpontaneousSetupPath_scene = slicer.modules.deliverytrainingspontaneoussetup.path.replace("DeliveryTrainingSpontaneousSetup.py","") + 'Resources/Data/rokoko_hands/'
    self.DeliveryTrainingSpontaneousSetupPath_scene = slicer.modules.deliverytrainingspontaneoussetup.path.replace("DeliveryTrainingSpontaneousSetup.py","") + 'Resources/Data/fixed_axis_vuforia_tracking/'
  ...
  #%% LOAD
    ...
    self.loadButtons.addWidget(self.connectToPlusButton)
     ...
     # Connect to unity
    # self.connectToUnityButton = qt.QPushButton('Connect To Unity')
    # self.connectToUnityButton.enabled = True
    # self.loadButtons.addWidget(self.connectToUnityButton)
...
#%% STEP 2. HOLD HEAD
    ...
    #%% STEP 2.2.  REMOVE HAND FROM PERINEUM -> incorporated in hold head step
    ...
    #%% STEP 4.2. PLACE BABY ON TOP OF MOTHER
  ...
  #%% --- CONNECTIONS ---
    ...
    # self.connectToUnityButton.connect('clicked(bool)', self.onConnectToUnityButtonClicked)
  ...
  def onLoadDataButtonClicked(self):
      ...
      try:
        ...
        try:
            ...
            scene_file = self.DeliveryTrainingSpontaneousSetupPath_scene + 'vuforia_tracking.mrml'
            # scene_file = self.DeliveryTrainingSpontaneousSetupPath_scene + '2_HANDS_COMPLETE.mrml'
        ...
        mother_reference = slicer.util.getNode("MARKER_6_T")
        
        # rhand_posterior = slicer.util.getNode("Marker4ToTracker")
        rhand_posterior = slicer.util.getNode("MARKER_3_T")
        
        rhand_anterior = slicer.util.getNode("MARKER_4_T")
        
        # lhand_posterior = slicer.util.getNode("Marker3ToTracker")
        lhand_posterior = slicer.util.getNode("MARKER_1_T")
        
        lhand_anterior = slicer.util.getNode("MARKER_2_T")
        ...
        # registro_izquierdo = slicer.util.getNode("registro izquierdo")
        registro_derecho = slicer.util.getNode("registro derecho")
        
        mother_model = slicer.util.getNode("MotherModel")
        axis_mother_model = slicer.util.getNode("axis_mother_model")
        
        
        axis_left_hand_model = slicer.util.getNode("axis_left_hand_model")
        axis_right_hand_model = slicer.util.getNode("axis_right_hand_model")
        
        last_right_hand_position = slicer.vtkMRMLLinearTransformNode()
        last_right_hand_position.SetName('last_right_hand_position')
        slicer.mrmlScene.AddNode(last_right_hand_position)
        
        last_right_hand_position_posterior = slicer.vtkMRMLLinearTransformNode()
        last_right_hand_position_posterior.SetName('last_right_hand_position_posterior')
        slicer.mrmlScene.AddNode(last_right_hand_position_posterior)
        
        last_right_hand_position_anterior = slicer.vtkMRMLLinearTransformNode()
        last_right_hand_position_anterior.SetName('last_right_hand_position_anterior')
        slicer.mrmlScene.AddNode(last_right_hand_position_anterior)
        
        last_left_hand_position = slicer.vtkMRMLLinearTransformNode()
        last_left_hand_position.SetName('last_left_hand_position')
        slicer.mrmlScene.AddNode(last_left_hand_position)
        
        last_left_hand_position_anterior = slicer.vtkMRMLLinearTransformNode()
        last_left_hand_position_anterior.SetName('last_left_hand_position_anterior')
        slicer.mrmlScene.AddNode(last_left_hand_position_anterior)
        
        last_left_hand_position_posterior = slicer.vtkMRMLLinearTransformNode()
        last_left_hand_position_posterior.SetName('last_left_hand_position_posterior')
        slicer.mrmlScene.AddNode(last_left_hand_position_posterior)

        # last_right_hand_position = slicer.mrmlScene.AddNode('last_right_hand_position')
        # last_left_hand_position = slicer.mrmlScene.AddNode('last_left_hand_position')

        
        RightHand_T.SetAndObserveTransformNodeID(axis_right_hand_model.GetID())
        axis_right_hand_model.SetAndObserveTransformNodeID(registro_derecho.GetID())
        
        # global last_right_hand_position
        # last_right_hand_position_matrix= vtk.vtkMatrix4x4()
        ...
        scale_factor= 10
        def mother_body_reference(caller,eventId):
            matrix_mother = mother_reference.GetMatrixTransformToParent()
            matrix_mother.SetElement(0, 3, matrix_mother.GetElement(0, 3) / scale_factor)
            matrix_mother.SetElement(1, 3, matrix_mother.GetElement(1, 3) / scale_factor)
            matrix_mother.SetElement(2, 3, matrix_mother.GetElement(2, 3) / scale_factor)
            mother_reference.SetMatrixTransformToParent(matrix_mother)
            
            mother_model.SetAndObserveTransformNodeID(axis_mother_model.GetID())
    
            axis_mother_model.SetAndObserveTransformNodeID(mother_reference.GetID())
        ...
        def right_hand_posterior(caller, eventId):
            ...
            for row in range(4):
                for col in range(4):
                    if row == col:
                        if matrix.GetElement(row, col) != 1.0:
                            is_identity = False
                            break
                    else:
                        if identity_matrix.GetElement(row, col) != 0.0:
                            is_identity = False
                            break
            last_right_hand_position_matrix = last_right_hand_position_posterior.GetMatrixTransformToParent()
            are_equal = True
            for row in range(4):
                for col in range(4):
                    if matrix.GetElement(row, col) != last_right_hand_position_matrix.GetElement(row, col):
                        are_equal = False
                        break

            if (not is_identity  and not are_equal):
                
                last_right_hand_position_posterior.SetMatrixTransformToParent(matrix)
              
                matrix.SetElement(0, 3, matrix.GetElement(0, 3) / scale_factor)
                matrix.SetElement(1, 3, matrix.GetElement(1, 3) / scale_factor)
                matrix.SetElement(2, 3, matrix.GetElement(2, 3) / scale_factor)
                
                
                last_right_hand_position.SetMatrixTransformToParent(matrix)
              
                rhand_posterior.SetMatrixTransformToParent(matrix)
                
                rightHand_T_matrix = RightHand_T.GetMatrixTransformToParent()
                rightHand_T_matrix.SetElement(0, 3, 0)
                rightHand_T_matrix.SetElement(1, 3, 0)
                rightHand_T_matrix.SetElement(2, 3, 0)
                
                RightHand_T.SetMatrixTransformToParent(rightHand_T_matrix)
                
                
                RightHand_T.SetAndObserveTransformNodeID(axis_right_hand_model.GetID())
                axis_right_hand_model.SetAndObserveTransformNodeID(None)
                axis_right_hand_model.SetAndObserveTransformNodeID(rhand_posterior.GetID())
        ...
        def right_hand_anterior(caller, eventId):
            ...
            last_right_hand_position_matrix = last_right_hand_position_anterior.GetMatrixTransformToParent()
            are_equal = True
            for row in range(4):
                for col in range(4):
                    if matrix.GetElement(row, col) != last_right_hand_position_matrix.GetElement(row, col):
            ...
            if (not is_identity  and not are_equal)  :
                ...
                last_right_hand_position_anterior.SetMatrixTransformToParent(matrix)
                ...
                axis_right_hand_model.SetAndObserveTransformNodeID(rhand_anterior.GetID())
        ...
        def left_hand_posterior(caller, eventId):
            ...
            last_left_hand_position_matrix = last_left_hand_position_posterior.GetMatrixTransformToParent()
            are_equal = True
            for row in range(4):
                for col in range(4):
                    if matrix.GetElement(row, col) != last_left_hand_position_matrix.GetElement(row, col):
                        are_equal = False
                        break

            if (not is_identity  and not are_equal)  :
                
                last_left_hand_position_posterior.SetMatrixTransformToParent(matrix)
                
                matrix.SetElement(0, 3, matrix.GetElement(0, 3) / scale_factor)
                matrix.SetElement(1, 3, matrix.GetElement(1, 3) / scale_factor)
                matrix.SetElement(2, 3, matrix.GetElement(2, 3) / scale_factor)
               
                last_left_hand_position.SetMatrixTransformToParent(matrix)
                
                lhand_posterior.SetMatrixTransformToParent(matrix)
                
                LeftHand_T_matrix = LeftHand_T.GetMatrixTransformToParent()
                LeftHand_T_matrix.SetElement(0, 3, 0)
                LeftHand_T_matrix.SetElement(1, 3, 0)
                LeftHand_T_matrix.SetElement(2, 3, 0)
                
                LeftHand_T.SetMatrixTransformToParent(LeftHand_T_matrix)
                
                LeftHand_T.SetAndObserveTransformNodeID(axis_left_hand_model.GetID())
                axis_left_hand_model.SetAndObserveTransformNodeID(lhand_posterior.GetID())
        ...
        def left_hand_anterior(caller, eventId):
            ...
            last_left_hand_position_matrix = last_left_hand_position_anterior.GetMatrixTransformToParent()
            are_equal = True
            for row in range(4):
                for col in range(4):
                    if matrix.GetElement(row, col) != last_left_hand_position_matrix.GetElement(row, col):
            ...
            if (not is_identity  and not are_equal)  :
                ...
                last_left_hand_position_anterior.SetMatrixTransformToParent(matrix)
                ...
                LeftHand_T.SetAndObserveTransformNodeID(axis_left_hand_model.GetID())
                axis_left_hand_model.SetAndObserveTransformNodeID(lhand_anterior.GetID())
                lhand_anterior.SetAndObserveTransformNodeID(axis_change_multiplication.GetID())
            
        # def right_hand_modified(caller, eventId):
            
        #     if (rhand_anterior == last_right_hand_position or rhand_posterior == last_right_hand_position):
        #         last_right_hand_position_matrix = last_right_hand_position.GetMatrixTransformToParent()
                
        #         last_right_hand_position_matrix.SetElement(0, 3, last_right_hand_position_matrix.GetElement(0, 3) / scale_factor)
        #         last_right_hand_position_matrix.SetElement(1, 3, last_right_hand_position_matrix.GetElement(1, 3) / scale_factor)
        #         last_right_hand_position_matrix.SetElement(2, 3, last_right_hand_position_matrix.GetElement(2, 3) / scale_factor)
                
        #         # last_position = [last_right_hand_position_matrix.GetElement(0, 3),
        #         #        last_right_hand_position_matrix.GetElement(1, 3),
        #         #        last_right_hand_position_matrix.GetElement(2, 3)]
                
        #         print('last position')
        #         print(last_right_hand_position_matrix)
                
        #         actual_matrix = RightHand_T.GetMatrixTransformToParent()
        #         # actual_position =[actual_matrix.GetElement(0, 3),
        #         #         actual_matrix.GetElement(1, 3),
        #         #         actual_matrix.GetElement(2, 3)]
        #         print('actual_matrix')
        #         print(actual_matrix)
        #         # # Calcula la diferencia entre la posición actual de Transformada 2 y la última posición registrada de Transformada 1
        #         # diferencia = [actual - last for actual, last in zip(actual_position, last_position)]
        #         # print('diferencia')
        #         # print(diferencia)
        #         # # Calcula la nueva posición de Transformada 2 basada en la última posición de Transformada 1 y la diferencia
        #         # # new_rightHand_position =  [last + dif for last, dif in zip(last_position, diferencia)]
        #         # new_rightHand_position =  diferencia
                
        #         # print('new_rightHand_position')
        #         # print(new_rightHand_position)
        #         # # Aplica la nueva posición a Transformada 2
        #         new_matrix = RightHand_T.GetMatrixTransformToParent()
        #         # new_matrix.SetElement(0, 3, new_rightHand_position[0])
        #         # new_matrix.SetElement(1, 3, new_rightHand_position[1])
        #         # new_matrix.SetElement(2, 3, new_rightHand_position[2])
        #         # RightHand_T.SetMatrixTransformToParent(new_matrix)
        #         # print('RightHand_T new matrix')
        #         # print(new_matrix)
        #         inverse_matrix2 = vtk.vtkMatrix4x4()
        #         vtk.vtkMatrix4x4.Invert(actual_matrix, inverse_matrix2)
        #         vtk.vtkMatrix4x4.Multiply4x4(last_right_hand_position_matrix, inverse_matrix2, new_matrix)
                
        #         RightHand_T.SetMatrixTransformToParent(new_matrix)
        #         print('RightHand_T new matrix')
        #         print(new_matrix)

        # def left_hand_modified(caller, eventId):
        #     last_left_hand_position_matrix = last_left_hand_position.GetMatrixTransformToParent()
         
        #     last_position = [last_left_hand_position_matrix.GetElement(0, 3),
        #             last_left_hand_position_matrix.GetElement(1, 3),
        #             last_left_hand_position_matrix.GetElement(2, 3)]
            
        #     actual_matrix = LeftHand_T.GetMatrixTransformToParent()
        #     actual_position =[actual_matrix.GetElement(0, 3),
        #             actual_matrix.GetElement(1, 3),
        #             actual_matrix.GetElement(2, 3)]
        ...
        #     # Calcula la nueva posición de Transformada 2 basada en la última posición de Transformada 1 y la diferencia
        #     new_leftHand_position =  [last + dif for last, dif in zip(last_position, diferencia)]
            
        #     # Aplica la nueva posición a Transformada 2
        #     new_matrix = LeftHand_T.GetMatrixTransformToParent()
        #     new_matrix.SetElement(0, 3, new_leftHand_position[0])
        #     new_matrix.SetElement(1, 3, new_leftHand_position[1])
        #     new_matrix.SetElement(2, 3, new_leftHand_position[2])
        #     LeftHand_T.SetMatrixTransformToParent(new_matrix)
        ...
        def right_hand_modified(caller, eventId):
            ...
            if (rhand_anterior == last_right_hand_position or rhand_posterior == last_right_hand_position):
                ...
                # dif_matrix = actual_matrix - last_right_hand_position_matrix
                
                # new_matrix = dif_matrix * actual_matrix.Invert()
                
                matriz_resultado = vtk.vtkMatrix4x4()
                # matriz_resultado.DeepCopy(last_right_hand_position_matrix)
                matriz_resultado.Multiply4x4(actual_matrix.Invert(), last_right_hand_position_matrix, matriz_resultado)
                ...
                RightHand_T.SetMatrixTransformToParent(matriz_resultado)
                
     
        def left_hand_modified(caller, eventId):
              last_left_hand_position_matrix = last_left_hand_position.GetMatrixTransformToParent()
         
              last_left_hand_position_matrix.SetElement(0, 3, last_left_hand_position_matrix.GetElement(0, 3) / scale_factor)
              last_left_hand_position_matrix.SetElement(1, 3, last_left_hand_position_matrix.GetElement(1, 3) / scale_factor)
              last_left_hand_position_matrix.SetElement(2, 3, last_left_hand_position_matrix.GetElement(2, 3) / scale_factor)
      
              actual_matrix = LeftHand_T.GetMatrixTransformToParent()
             
              # dif_matrix = actual_matrix - last_left_hand_position_matrix
             
              # new_matrix = dif_matrix * actual_matrix.Invert()
              matriz_resultado = vtk.vtkMatrix4x4()
              # matriz_resultado.DeepCopy(last_left_hand_position_matrix)
              matriz_resultado.Multiply4x4(actual_matrix.Invert(), last_left_hand_position_matrix, matriz_resultado)
            ...
            # Aplica la nueva posición a Transformada 2
              ...
              # new_matrix.SetElement(1, 3, new_leftHand_position[1])
              # new_matrix.SetElement(2, 3, new_leftHand_position[2])
              LeftHand_T.SetMatrixTransformToParent(matriz_resultado)
        ...
        mother_reference.AddObserver('ModifiedEvent', mother_body_reference)
        
        RightHand_T.AddObserver('ModifiedEvent', right_hand_modified)
        LeftHand_T.AddObserver('ModifiedEvent', left_hand_modified)
...
#%% onConnectToPlusButtonClicked
  def onConnectToPlusButtonClicked(self):
     logging.debug('Connecting to plus and unity')

     if self.connect:
         try: 
             cnode = slicer.util.getNode('IGTLConnector')
             print('connected to plus')
         except:
...
#%% onConnectToUnityButtonClicked
  def onConnectToUnityButtonClicked(self):
      logging.debug('Connecting to plus and unity')

      if self.connect_unity:
        
        try: 
            cnode_unity = slicer.util.getNode('IGTLConnector_unity')
            print("connected ")
        except:
            cnode_unity = slicer.vtkMRMLIGTLConnectorNode()
            slicer.mrmlScene.AddNode(cnode_unity)
            cnode_unity.SetName('IGTLConnector_unity')
        correct_unity = cnode_unity.SetTypeServer(18945)
        print(correct_unity)
        if correct_unity == 1:
            cnode_unity.Start()
            logging.debug('Successful connection')
            self.connect_unity = False
            self.connectToUnityButton.setText('Disconnect from Unity')
            self.applyTransformsButton.enabled = True
        else:
          print('Unable to connect to Unity')
          logging.debug('Unable to connect to Unity')
      else:
        cnode_unity = slicer.util.getNode('IGTLConnector_unity')
        cnode_unity.Stop()
        self.connect_unity = True
        self.connectToUnityButton.setText('Connect toUnity')
    
    
        

#%% onApplyTransformsButtonClicked
  def onApplyTransformsButtonClicked(self):
    ...
    #   if self.firstTime:
        ...
        # self.addSWatchedNode(wd, self.handRToTracker, warningMessage = 'HandR is out of view', playSound = True)
    ...
    try:
      ...
      # logging.debug('Setting transform tree')
       # Hand L
        # self.left_hand.SetAndObserveTransformNodeID(self.handLModelToHandL.GetID())
        ...
        # # Hand R
        # self.mano_derecha.SetAndObserveTransformNodeID(self.handRModelToHandR.GetID())
...
# DEFLEXION
  ...
  def onStartDeflexionClicked(self):
      ...
      if self.rightHandedRadioButton.isChecked(): 
        ...
        toolToReference = 'last_right_hand_position' #Dominant hand
        
      else:
        # toolToReference = 'HandLToTracker'
        toolToReference = 'last_left_hand_position'
...
class HAND_TRACKINGLogic(ScriptedLoadableModuleLogic):
...
#%%
  ...
  def saveSoftwareActivity(self):
    ...
    #         matrix.SetElement(0, 3, matrix.GetElement(0, 3) / self.scale_factor)
    #         matrix.SetElement(1, 3, matrix.GetElement(1, 3) / self.scale_factor)
    #         matrix.SetElement(2, 3, matrix.GetElement(2, 3) / self.scale_factor)
    ...
    #         last_right_hand_position_matrix.SetElement(0, 3, last_right_hand_position_matrix.GetElement(0, 3) / self.scale_factor)
    #         last_right_hand_position_matrix.SetElement(1, 3, last_right_hand_position_matrix.GetElement(1, 3) / self.scale_factor)
    #         last_right_hand_position_matrix.SetElement(2, 3, last_right_hand_position_matrix.GetElement(2, 3) / self.scale_factor)
    ...
    #      last_left_hand_position_matrix.SetElement(0, 3, last_left_hand_position_matrix.GetElement(0, 3) / self.scale_factor)
    #      last_left_hand_position_matrix.SetElement(1, 3, last_left_hand_position_matrix.GetElement(1, 3) / self.scale_factor)
    #      last_left_hand_position_matrix.SetElement(2, 3, last_left_hand_position_matrix.GetElement(2, 3) / scalself.scale_factore_factor)
  
    #      actual_matrix = LefttHand_T.GetMatrixTransformToParent()
    ...
    try:
       ...
       #  identity_matrix.Identity()
        ...
        try:
          ...
          print('Marker_6_T not found')
        return False
        motherToTracker_matrix = motherToTracker.GetMatrixTransformToParent()
        # .SetMatrixTransformToParent(matrix_mother)
        
        # diff_matrix_left = last_left_hand_position_matrix - motherToTracker_matrix
      ...
      # get the trackerToTool transform for the ToolModelToTool transform
        ...
        if transformName == 'BabyHeadModelToBabyHead':
          ...
          modelName = ['MotherModelToMother','MotherTummyModel', 'IschialPlane', 'motherRealisticModel', 'sacrumLeftModel', 'sacrumRightModel',
      ...
      #       trackerTransform_inv.GetID())
        ...
        if transformName != 'MotherModelToMother':
          mother_reference.SetAndObserveTransformNodeID(
              trackerTransform_inv.GetID())
...
class HAND_TRACKINGTest(ScriptedLoadableModuleTest):
  """
  ...
  def runTest(self):
    ...
    """
    self.setUp()
    self.test_HAND_TRACKING1()

  def test_HAND_TRACKING1(self):
    ...
    # import SampleData
    # registerSampleData()
    # inputVolume = SampleData.downloadSample('HAND_TRACKING1')
